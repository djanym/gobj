...
class db {
	 ...
	 * against the required MySQL version for Gobj. Normally, a replacement
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		 ...
		 *  - We are a development version of Gobj, or
	...
	/**
	 * Change the current SQL mode, and ensure its Gobj compatibility.
	 *
...
//		wp_load_translations_early();
		...
		if ( $caller = $this->get_caller() ) {
			...
			$error_str = sprintf( __( 'Gobj database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query */
			$error_str = sprintf( __( 'Gobj database error %1$s for query %2$s' ), $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: support forums URL */
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">Gobj Support Forums</a>.' ),
...
//		$charset = apply_filters( 'pre_get_col_charset', null, $table, $column );
	...
	public function check_database_version() {
		...
		if ( version_compare($this->db_version(), $required_mysql_version, '<') ) {
			/* translators: 1: Gobj version number, 2: Minimum required MySQL version number */
			return new Error('database_version', sprintf( __( '<strong>ERROR</strong>: Gobj %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ));
		}
	}
